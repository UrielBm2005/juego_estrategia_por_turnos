...
obstaculos = [
    {'x': 2, 'y': 7},{'x': 2, 'y': 8}, {'x': 2, 'y': 9},{'x': 2, 'y': 10},{'x': 2, 'y': 20},
    {'x': 3, 'y': 1}, {'x': 3, 'y': 2}, {'x': 3, 'y': 3},{'x': 3, 'y': 7},{'x': 3, 'y': 10},
    {'x': 4, 'y': 3},{'x': 4, 'y': 10},
    {'x': 5, 'y': 10},
    {'x': 6, 'y': 10},
    {'x': 8, 'y': 2},{'x': 8, 'y': 3},
    ...
    {'x': 11, 'y': 1},{'x': 11, 'y': 5}, {'x': 11, 'y': 6},
    {'x': 12, 'y': 6},{'x': 12, 'y': 7},
    {'x': 13, 'y': 6},
    {'x': 14, 'y': 11},
    {'x': 15, 'y': 11},
    {'x': 16, 'y': 11},
    {'x': 17, 'y': 1},{'x': 17, 'y': 2},{'x': 17, 'y': 10},{'x': 17, 'y': 11},
    {'x': 18, 'y': 2},{'x': 18, 'y': 8},{'x': 18, 'y': 13},
    {'x': 19, 'y': 8},{'x': 19, 'y': 9},{'x': 19, 'y': 10},{'x': 19, 'y': 13},
    {'x': 20, 'y': 2},{'x': 20, 'y': 3},{'x': 20, 'y': 4},{'x': 20, 'y': 5},
]

def seleccionar_personaje_o_enemigo(x, y):
    global personaje_seleccionado, enemigo_seleccionado
    # Verificar si el clic ocurrió sobre un jugador
    for i, jugador in enumerate(jugadores):
        if jugador['x'] == x and jugador['y'] == y:
            personaje_seleccionado = i
            return
    # Verificar si el clic ocurrió sobre un enemigo
...
def mover_enemigo_a_posicion(x, y):
    global enemigo_seleccionado
    if enemigo_seleccionado is not None:
        enemigo = enemigos[enemigo_seleccionado]
        # Verificar si la nueva posición es adyacente y disponible
        if (abs(x - enemigo['x']) <= 1 and abs(y - enemigo['y']) <= 1 and
            not is_position_occupied(x, y)):
            enemigo['x'] = x
            enemigo['y'] = y
            enemigo_seleccionado = None # Resetear la selección del enemigo después de moverlo

def dibujar_casillas_disponibles_enemigo():
    if enemigo_seleccionado is not None:
        enemigo = enemigos[enemigo_seleccionado]
        for i in range(enemigo['x'] - 1, enemigo['x'] + 2):
            for j in range(enemigo['y'] - 1, enemigo['y'] + 2):
                if 0 <= i < grid_columns and 0 <= j < grid_rows and not is_position_occupied(i, j):
...
def game_loop():
    global turno_jugador, turno_enemigo, personaje_seleccionado, enemigo_seleccionado
    running = True
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                y //= grid_size
                if turno_jugador:
                    seleccionar_personaje_o_enemigo(x, y)
                    mover_personaje(x, y)
                elif turno_enemigo and enemigo_seleccionado is not None:
                    mover_enemigo_a_posicion(x, y)
                    enemigo_seleccionado = None # Resetear la selección del enemigo después de moverlo
        ...
        # Cambiar de turno al final de cada iteración del bucle
        if turno_jugador:
            turno_jugador = False
            turno_enemigo = True
        else:
        ...
        # Dibujar las casillas disponibles para el enemigo si es su turno
        if turno_enemigo and enemigo_seleccionado is not None:
            dibujar_casillas_disponibles_enemigo()
